...
**Remark***:
Dynamic objects are removed by Garbage Collector when they have expired, there is no phase of hte objct being a tombstone when it has expired. You need to be careful with the AD replication latency when you refresh objects. You have to ensure that the refresh happens early enough so all replicas of the naming context (full and global catalog) see the refresh transaction before the objec expires locally.
